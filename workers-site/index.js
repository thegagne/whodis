...
import { dnsLookup } from './helpers/dns'
...
async function handleEvent(event) {
  ...
  if (url.pathname.startsWith('/api/domain')) {
    try {
      ...
      const domainInfo = {};
      domainInfo.ns = await dnsLookup(domain, 'NS')
      domainInfo.mx = await dnsLookup(domain, 'MX')
      domainInfo.txt = await dnsLookup(domain, 'TXT')
